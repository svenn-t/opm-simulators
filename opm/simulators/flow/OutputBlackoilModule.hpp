            ...
            if (this->mech_.allocated()) {
                this->extractors_.push_back(
                    Entry{[&mech = this->mech_,
                            &model = simulator_.problem().geoMechModel(),
                            &faceStressInf = this->simulator_.problem().geoMechModel().linearizer().
                            getStressInfo()](const Context& ectx) {
                            ...
                            mech.assignPotentialForces(ectx.globalDofIdx,
                                                       ...
                                                       model.mechPotentialPressForce(
                                                           ectx.globalDofIdx),
                                                       model.mechPotentialTempForce(
                                                           ectx.globalDofIdx));
                            ...
                            mech.assignStress(ectx.globalDofIdx,
                                              model.stress(ectx.globalDofIdx,
                                                           /*include_fracture*/
                                                           true));

                            if (!faceStressInf.empty()) {
                                const auto& faceStressInfos = faceStressInf[ectx.globalDofIdx];
                                for (const auto& fstressInfo : faceStressInfos) {
                                    mech.assignFaceStress(ectx.globalDofIdx,
                                                          fstressInfo.faceId,
                                                          fstressInfo.stress);
                                }
                            }
