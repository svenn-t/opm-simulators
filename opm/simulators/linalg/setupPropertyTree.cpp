                  ...
                  bool tpsaSetup)
{
    ...
    // Setup TPSA block precoditioners
    if (tpsaSetup) {
        return setupTpsa(conf, p);
    }

    // Use CPR configuration.
    if ((conf == "cpr_trueimpes") || (conf == "cpr_quasiimpes") || (conf ==
        "cpr_trueimpesanalytic")) {
...
PropertyTree
setupTpsa(const std::string& conf, const FlowLinearSolverParameters& p)
{
    using namespace std::string_literals;
